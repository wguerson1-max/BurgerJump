...
function checkOrientation() {
  ...
  rotateMsg.style.display = vertical ? "flex" : "none";
...
function spawn() {
  ...
  let f = foodTypes[rand(0, foodTypes.length - 1)];

  // menos corazones
  if (f.type === "heart" && Math.random() > 0.05) {
    return;
  }
...
function update() {
  ...
  const spawnRate = rand(30, 90) / difficulty;
  ...
  for (let i = items.length - 1; i >= 0; i--) {
    ...
    if (hit(player, it)) {
      ...
      if (d.points) {
        ...
        const pts = d.points * scoreMultiplier;
        ...
        if (lastFoods.length > 3) {
          lastFoods.shift();
        }

        if (
          lastFoods.length === 3 &&
          lastFoods[0] === lastFoods[1] &&
          lastFoods[1] === lastFoods[2]
        ) {
...
function render() {
  ...
  // score
  ctx.drawImage(scoreImg, 20, 10, 30, 30);

  ctx.fillText(score, 60, 32);

  // best
  ctx.drawImage(bestImg, 20, 45, 30, 30);

  ctx.fillText(highScore, 60, 67);
